import { useCurrentApp } from '@/context/app.context';
import { getURLBaseBackend, processDataRestaurantMenu, getBooksByBookStoreAPI, currencyFormatter } from '@/utils/api';
import { APP_COLOR } from '@/utils/constant';
import MaterialIcons from '@expo/vector-icons/MaterialIcons';
import React, { useRef, useState, useEffect } from 'react';
import { Dimensions, FlatList, Image, SectionList, StyleSheet, Text, TouchableOpacity, View, ViewToken, Pressable } from 'react-native';
import Animated, { Extrapolation, interpolate, interpolateColor, useAnimatedScrollHandler, useAnimatedStyle, useSharedValue } from 'react-native-reanimated';
import { router } from 'expo-router';
import Info from './info';
import ItemQuantity from './order/item.quantity';
import StickyFooter from './order/sticky.footer';
import StickyHeader from './sticky.header';


const AnimatedSectionList = Animated.createAnimatedComponent(SectionList);

const { height: sHeight, width: sWidth } = Dimensions.get('window');

const HEADER_HEIGHT = 120;
const IMAGE_HEIGHT = 220;
const INFO_HEIGHT = 250;
const SLIDE_MENU_HEIGHT = 50;

const RMain = () => {

    const { restaurant, cart, setCart } = useCurrentApp()

    const scrollY = useSharedValue(0);

    const sectionListRef = useRef<SectionList>(null);
    const flatListRef = useRef<FlatList>(null);
    const [activeMenuIndex, setActiveMenuIndex] = useState<number | string>(0);
    const blockUpdateRef = useRef<boolean>(false);
    const [books, setBooks] = useState<IBook[]>([]);
    const [loadingBooks, setLoadingBooks] = useState(false);

    // Scroll handler to update the scrollY value
    const onScroll = useAnimatedScrollHandler((event) => {
        scrollY.value = event.contentOffset.y;
        // console.log(scrollY.value)
    });


    // Fade-in effect for the restaurant header
    const animatedStickyHeaderStyle = useAnimatedStyle(() => {
        const opacity = interpolate(
            scrollY.value,
            [0, 100],
            [0, 1],
            Extrapolation.CLAMP
        );
        const pointerEvents = opacity === 0 ? 'none' : 'auto';

        return {
            opacity,
            pointerEvents //on/off click input
        };
    });

    // Sticky positioning for the menu below the header
    const animatedMenuStyle = useAnimatedStyle(() => {
        const range = IMAGE_HEIGHT + INFO_HEIGHT - HEADER_HEIGHT;
        const translateY = interpolate(
            scrollY.value,
            [0, range], // Define scroll range
            [0, -range - 2], //2px menu border
            Extrapolation.CLAMP
        );

        return {
            transform: [{ translateY }],
            position: 'absolute',
            top: IMAGE_HEIGHT + INFO_HEIGHT,
            zIndex: 2,
            width: '100%',
            backgroundColor: 'white',
        };
    });

    const animatedInfoStyle = useAnimatedStyle(() => {
        const range = IMAGE_HEIGHT + INFO_HEIGHT - HEADER_HEIGHT;

        const translateY = interpolate(
            scrollY.value,
            [0, range], // Define scroll range
            [0, -range],
            Extrapolation.CLAMP
        );

        return {
            transform: [{ translateY }],
            position: 'absolute',
            top: IMAGE_HEIGHT,
            zIndex: 1,
            width: '100%',
        };
    });

    const animatedHeartIconStyle = useAnimatedStyle(() => {
        const range = IMAGE_HEIGHT + INFO_HEIGHT - HEADER_HEIGHT;

        const translateY = interpolate(
            scrollY.value,
            [0, range], // Define scroll range
            [0, -range],
            Extrapolation.CLAMP
        );

        return {
            transform: [{ translateY }],

        };
    });

    // Animated styles for background
    const animatedBackgroundStyle = useAnimatedStyle(() => {
        return {
            backgroundColor: interpolateColor(
                scrollY.value,
                [0, 100],
                ['rgba(0,0,0,0.3)', 'transparent'],
            ),
        };
    });

    // Animate arrow color
    const animatedArrowColorStyle = useAnimatedStyle(() => {
        return {
            color: interpolateColor(
                scrollY.value,
                [0, 100],
                ['white', APP_COLOR.ORANGE], // Arrow color range
            ),
        };
    });


    // Fetch books when restaurant is loaded
    useEffect(() => {
        const fetchBooks = async () => {
            if (restaurant?._id) {
                setLoadingBooks(true);
                try {
                    console.log('>>Fetching books for bookstore:', restaurant._id);
                    const res = await getBooksByBookStoreAPI(restaurant._id, 1, 20);
                    console.log('>>Books response:', res);
                    
                    let booksData: IBook[] = [];
                    
                    // Handle different response structures (axios interceptor returns response.data)
                    if (res?.data?.data?.results && Array.isArray(res.data.data.results)) {
                        // Structure: { data: { data: { results: [...] } } }
                        booksData = res.data.data.results;
                    } else if (res?.data?.results && Array.isArray(res.data.results)) {
                        // Structure: { data: { results: [...] } }
                        booksData = res.data.results;
                    } else if (Array.isArray(res?.data)) {
                        // Structure: { data: [...] }
                        booksData = res.data;
                    } else if (Array.isArray(res)) {
                        // Structure: [...] (direct array)
                        booksData = res;
                    }
                    
                    console.log('>>Processed books:', booksData.length);
                    setBooks(booksData);
                } catch (error: any) {
                    console.error('>>Error fetching books:', error);
                    console.error('>>Error details:', {
                        message: error?.message,
                        response: error?.response?.data,
                        status: error?.response?.status
                    });
                } finally {
                    setLoadingBooks(false);
                }
            }
        };
        fetchBooks();
    }, [restaurant?._id]);

    const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: Array<ViewToken> }) => {
        if (viewableItems.length > 0 && !blockUpdateRef.current) {
            const visibleSectionIndex = viewableItems[0].section.index;
            setActiveMenuIndex(visibleSectionIndex)
            flatListRef.current?.scrollToIndex({ index: visibleSectionIndex, animated: true });
        }
    }).current;

    // Render books footer component
    const renderBooksFooter = () => {
        if (loadingBooks) {
            return (
                <View style={styles.booksSection}>
                    <Text style={styles.booksTitle}>Đang tải sách...</Text>
                </View>
            );
        }

        if (books.length === 0) return null;

        return (
            <View style={styles.booksSection}>
                <View style={styles.booksHeader}>
                    <Text style={styles.booksTitle}>Sách có thể bạn quan tâm</Text>
                </View>
                {books.map((item) => {
                    const handleAddBook = () => {
                        if (!restaurant?._id) return;

                        const book: IMenuItem = {
                            ...item,
                            basePrice: item.price,
                        } as IMenuItem;

                        if (!cart[restaurant._id]) {
                            cart[restaurant._id] = {
                                sum: 0,
                                quantity: 0,
                                items: {}
                            };
                        }

                        const itemPrice = book.basePrice || book.price || 0;
                        cart[restaurant._id].sum = cart[restaurant._id].sum + itemPrice;
                        cart[restaurant._id].quantity = cart[restaurant._id].quantity + 1;

                        if (!cart[restaurant._id].items[book._id]) {
                            cart[restaurant._id].items[book._id] = {
                                data: book,
                                quantity: 0
                            };
                        }

                        const currentQuantity = cart[restaurant._id].items[book._id].quantity + 1;
                        cart[restaurant._id].items[book._id] = {
                            data: book,
                            quantity: currentQuantity
                        };

                        setCart((prevState: any) => ({ ...prevState, ...cart }));
                    };

                    const currentQuantity = restaurant?._id && cart[restaurant._id]?.items[item._id]?.quantity || 0;

                    return (
                        <View key={item._id} style={styles.bookItem}>
                            <View style={styles.bookItemContent}>
                                <Text style={styles.bookItemTitle}>{item.title}</Text>
                                <View style={styles.bookItemFooter}>
                                    <Text style={styles.bookItemPrice}>
                                        {currencyFormatter(item.price || 0)}
                                    </Text>
                                    <Pressable
                                        onPress={handleAddBook}
                                        style={styles.addButton}
                                    >
                                        <MaterialIcons
                                            name="add"
                                            size={24}
                                            color="white"
                                        />
                                    </Pressable>
                                </View>
                            </View>
                        </View>
                    );
                })}
            </View>
        );
    };

    return (
        <View style={{ flex: 1 }}>
            <StickyHeader
                headerHeight={HEADER_HEIGHT}
                imageHeight={IMAGE_HEIGHT}
                animatedBackgroundStyle={animatedBackgroundStyle}
                animatedArrowColorStyle={animatedArrowColorStyle}
                animatedStickyHeaderStyle={animatedStickyHeaderStyle}
                animatedHeartIconStyle={animatedHeartIconStyle}
            />


            <View style={styles.header}>
                <Image
                    source={{ uri: `${getURLBaseBackend()}/images/restaurant/${restaurant?.image}` }}
                    style={styles.headerImage}
                />
            </View>


            <Animated.View style={[animatedInfoStyle]}>
                <Info
                    infoHeight={INFO_HEIGHT}
                    restaurant={restaurant}
                />
            </Animated.View>


            <Animated.FlatList
                ref={flatListRef}
                horizontal
                data={processDataRestaurantMenu(restaurant)}
                renderItem={({ item, index }) => (
                    <TouchableOpacity key={index}
                        onPress={() => {
                            blockUpdateRef.current = true;
                            setActiveMenuIndex(index)
                            sectionListRef.current?.scrollToLocation({
                                sectionIndex: item.index,
                                itemIndex: 0,
                                viewOffset: HEADER_HEIGHT + SLIDE_MENU_HEIGHT
                            })
                        }}
                    >
                        <View style={{
                            paddingHorizontal: 7,
                            height: SLIDE_MENU_HEIGHT,
                            justifyContent: "center",
                            borderBottomColor: item.index === activeMenuIndex ? APP_COLOR.ORANGE : APP_COLOR.GREY,
                            borderBottomWidth: 2
                        }}>
                            <Text style={{
                                color: item.index === activeMenuIndex ? APP_COLOR.ORANGE : "black",
                                marginHorizontal: 5,
                            }}>{item.title}</Text>
                        </View>
                    </TouchableOpacity>
                )}
                showsHorizontalScrollIndicator={false}
                style={[animatedMenuStyle]}
            />

            <AnimatedSectionList
                ref={sectionListRef as any}
                style={{ zIndex: 1 }}
                onScroll={onScroll}
                stickySectionHeadersEnabled={false}
                contentContainerStyle={{
                    paddingTop: IMAGE_HEIGHT + INFO_HEIGHT + SLIDE_MENU_HEIGHT - 2,
                    paddingBottom: 50,
                }}
                sections={processDataRestaurantMenu(restaurant)}
                renderItem={({ item, index }: { item: any, index: any }) => {
                    const menuItem = item as IMenuItem
                    return (<ItemQuantity
                        restaurant={restaurant}
                        menuItem={menuItem}
                        isModal={false}
                    />)
                }
                }
                renderSectionHeader={({ section }: { section: any }) => (

                    <View style={{ backgroundColor: "white", paddingHorizontal: 10, paddingTop: 10 }}>
                        <Text style={{ textTransform: "uppercase" }}>{section.title}</Text>
                    </View>
                )}

                ItemSeparatorComponent={() => (<>
                    <View style={{ backgroundColor: "white", paddingHorizontal: 10 }}>
                        <View style={{ height: 1, backgroundColor: "#ccc", marginVertical: 5 }} />
                    </View>
                </>)}
                viewabilityConfig={{
                    viewAreaCoveragePercentThreshold: 1,
                    waitForInteraction: true,
                }}
                onViewableItemsChanged={onViewableItemsChanged}
                onMomentumScrollEnd={() => (blockUpdateRef.current = false)}
                ListFooterComponent={renderBooksFooter}
            />

            <StickyFooter restaurant={restaurant} />
        </View>
    );
};

const styles = StyleSheet.create({
    container: {
        flex: 1,
    },
    menuItem: {
        fontSize: 16,
        fontWeight: 'bold',
        color: APP_COLOR.ORANGE,
        marginHorizontal: 10,
    },
    header: {
        width: sWidth,
        height: IMAGE_HEIGHT,
        top: 0,
        left: 0,
        position: 'absolute',
        zIndex: 1
    },
    headerImage: {
        width: '100%',
        height: '100%',
        resizeMode: 'cover',
    },
    booksSection: {
        backgroundColor: 'white',
        paddingTop: 20,
        paddingBottom: 20,
        paddingHorizontal: 10,
    },
    booksHeader: {
        marginBottom: 15,
    },
    booksTitle: {
        fontSize: 18,
        fontWeight: 'bold',
        color: APP_COLOR.ORANGE,
    },
    bookItem: {
        backgroundColor: 'white',
        paddingHorizontal: 10,
        paddingVertical: 15,
        borderBottomWidth: 1,
        borderBottomColor: '#eee',
    },
    bookItemContent: {
        flexDirection: 'column',
        gap: 8,
    },
    bookItemTitle: {
        fontSize: 16,
        color: '#333',
        fontWeight: '400',
    },
    bookItemFooter: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        alignItems: 'center',
    },
    bookItemPrice: {
        fontSize: 16,
        fontWeight: '600',
        color: APP_COLOR.ORANGE,
    },
    addButton: {
        backgroundColor: APP_COLOR.ORANGE,
        width: 32,
        height: 32,
        borderRadius: 4,
        justifyContent: 'center',
        alignItems: 'center',
    },
});

export default RMain;